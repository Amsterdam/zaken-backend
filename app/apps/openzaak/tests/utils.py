...
class OpenZaakBaseMixin:
    def setUp(self):
        super().setUp()

        self.ZAKEN_ROOT = "https://zaken.nl/api/v1/"
        self.DOCUMENTEN_ROOT = "https://documenten.nl/api/v1/"
        self.CATALOGI_ROOT = "http://localhost:8000/catalogi/api/v1/"
        self.NOTIFICATION_ROOT = "https://notification.nl/api/v1/"

        self.ZAAK_TYPE_URL = f"{self.CATALOGI_ROOT}zaaktypen/e59108b0-ed10-41f8-bbbe-cb7e5546d54c"
        self.STATUS_TYPE_URL = f"{self.CATALOGI_ROOT}statustypen/a5628108-456f-4459-9c9c-4be8c9f67f13"
        self.ZAAK_URL = f"{self.ZAKEN_ROOT}zaken/d8bbdeb7-770f-4ca9-b1ea-77b4730bf67d"
        self.STATUS_URL = f"{self.ZAKEN_ROOT}statussen/f0875a2d-c128-440e-98cd-a0a2b1e36cad"
        ...
        baker.make(
            ...
            api_root=self.CATALOGI_ROOT,
            client_id="aza_amsterdam",
            secret="12345",
            oas=f"{self.CATALOGI_ROOT}schema/openapi.yaml",
        )
        baker.make(
            Service,
            api_type=APITypes.drc,
            api_root=self.DOCUMENTEN_ROOT,
            client_id="aza_amsterdam",
            secret="12345",
            oas=f"{self.DOCUMENTEN_ROOT}schema/openapi.yaml",
        )
        baker.make(
            Service,
            api_type=APITypes.zrc,
            api_root=self.ZAKEN_ROOT,
            client_id="aza_amsterdam",
            secret="12345",
            oas=f"{self.ZAKEN_ROOT}schema/openapi.yaml",
        )
        ...
        self.informatieobjecttypen = paginated_response([self.informatie_type, self.informatie_type2])
